...
**				main2.c. It has functions that will simulate a heap. 
** Input: none
** Output: none
*********************************************************************/
#include <assert.h>
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (rghtIdx < max) //See if there is two children
	{
		smallestChild = _smallerIndexHeap(heap, leftIdx, rghtIdx);
		
		if (compare(getDynArr(heap, smallestChild), getDynArr(heap, pos)) == -1);		// use compare function to check if they left is bigger
		{
			//There is two children, find the smallest
			swapDynArr(heap, pos, smallestChild);			// swap the smallest with the pos
			_adjustHeap(heap, max, smallestChild);			// adjust heap
		}
	}
...
{
	int last;
	assert(sizeDynArr(heap) > 0); /* make sure we have at least one element */
...
void _buildHeap(DynArr *heap)
{
	...
	for (i = max / 2 - 1; i >= 0; i--)				// adjust heap from half of max -1
	{
		_adjustHeap(heap, max, i);
	}
	for (i = (max - 2) / 2; i >= 0; i--)			// adjust heap again to make sure it is fully adjusted
	{
		_adjustHeap(heap, max, i);
	}
}
...
void sortHeap(DynArr *heap)
{
	...
	for (i = sizeDynArr(heap) - 1; i > 0; i--)			// work backwards to sort from largest to smallest
	{
		swapDynArr(heap, 0, i);
