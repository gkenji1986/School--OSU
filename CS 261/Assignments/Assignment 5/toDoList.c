...
**				main.c. It has functions that allow us to add, remove, print, etc tasks
**				to a heap and a file.
** Input: none
** Output: none
*********************************************************************/
#include <stdio.h>
...
{
	struct Task *newTask;				// Create a task struct

	newTask = (struct Task *)malloc(sizeof(struct Task));		// Allocate memory for the new node

	assert(newTask != 0);					// check if allocated properly

	strcpy(newTask->description, desc);		// Assign the attributes
...
int compare(TYPE left, TYPE right)
{
	...
	data1 = (Task*)left;						// make temp1 equal to left
	data2 = (Task*)right;					// make temp2 equal to right

	if (data1->priority < data2->priority)				// check if the left number is less than the right
	{
		return -1;
	}
	else if (data1->priority > data2->priority)			// check if the left number is less than the right
	{
		return 1;
	}
	else                                            // if equal
