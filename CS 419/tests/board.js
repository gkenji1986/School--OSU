module.exports = function() {
    var board = {};
    board.screenToRowCol = function(x, y) {
        var col = Math.floor(x / this.tileWidth);
        var row = Math.floor(y / this.tileHeight);
        return { col: col, row: row };
    };
    board.rowColToScreen = function (row, col) {
        var x = this.tileWidth * (col); // +0.5 for tile center
        var y = this.tileHeight * (row);
        return { x: x, y: y };
    };

    board.areEnemiesCleared = function () {
        return (typeof this.enemyList === "object" && this.enemyList.length === 0);
    }
    // Summon new piece on board
    board.addBuildingAtTile = function (buildingClass, col, row) {
    ...
    board.addEnemy = function (enemyClass) {
    ...
    board.isUndefined = function(_arr, _index1, _index2) {
        try {
    ...
    board.checkWithinBounds = function(col, row) {
        if (col < 0 || col > this.cols || row < 0 || row > this.rows) {
            return false;
        }
        return true;
    }
    board.surroundingNodes = function(mapRef, enemyRef) {
        var nodes = new Array();
        var col, row;
    ...
    board.generateMatrix = function (ref) {
...
/*        back = new Shape();
    ...
    // Event Handlings
    board.readyToPlaceBuilding = function (e) {
    ...
    board.buildingPlacementCheck = function (e) {
    ...
    board.addBullet = function (bullet) {
        game.effectLayer.addChild(bullet);
        this.bulletList.push(bullet);
    };

    board.removeAllBullets = function () {
